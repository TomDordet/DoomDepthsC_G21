...
st_player * fight_monsters_round(st_player* p_player, st_monsters* p_first_monster)
{
    ...
    while(p_monster != NULL){ // tant que y'a des monstres :
        if(p_monster->currentLife > 0){ // et que le monstre est vivant ;
            ...
            printf("Le monstre %d inflige %d dammage au player (%d/%d)\n", p_monster->number, p_monster->attack, p_player->currentLife, p_player->maxLife);
        }
        p_monster = (st_monsters *)p_monster->p_next; // i++
    }

    if(p_player->currentLife < 0) p_player->currentLife = 0; // on remet à 0 pour pas avoir de valeurs négatives
