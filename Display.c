...
void display_mana_bar(st_player *p_player) {
    int currentMana = p_player->currentMana;
    int maxMana = p_player->maxMana;
    // Longueur de la barre de vie
    int barLength = 100;

    int manaBarSize = (currentMana * barLength) / maxMana;
    printf("Mana: [");
    for (int i = 0; i < barLength; i++) {
        if (i < manaBarSize) {
            printf("\x1b[34m#\x1b[0m");  // Utiliser le code ANSI pour la couleur bleue
        } else {
            printf("\x1b[34m-\x1b[0m");  // Utiliser le code ANSI pour la couleur bleue
        }
    }
...
void display_gold(st_player *p_player) {
    int gold = p_player->gold;
    printf("gold : \x1b[33m%d\x1b[0m\n", gold);
}

void barreAffichage(st_player *p_player){
    display_health_bar(p_player);
    display_mana_bar(p_player);
    display_gold(p_player);
}
